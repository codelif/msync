...
from src.youtube.downloader import downloader
from src.youtube.config import validate_config
...
parser.add_argument(
    "-o", "--output-folder", metavar="PATH", type=str, help="Override output folder."
)
parser.add_argument(
    "-f",
    "--force-update",
...
def update(playlist: dict):
    ...
    local_songs = [
        os.path.join(playlist_folder, video) for video in os.listdir(playlist_folder)
    ]
    ...
    if len(net_change) == 0:
        ...
        print(
            "Changes detected! {} songs added and {} songs removed in upstream. Syncing...".format(
...
def main():
    ...
    for playlist in playlists:
        if (
            playlist["id"] in list(sync_prev.keys())
            and os.path.exists(
                os.path.join(MUSIC_DIRECTORY, f"{playlist['title']} (Youtube)")
            )
            and not argv.force_update
        ):
            # update
            print(
                "The playlist '%s' has been previously synced. Detecting changes..."
                % playlist["title"]
            )
            update(playlist)
            ...
            if argv.force_update:
                ...
                print(
                    "The playlist '%s' has not been previously synced. Syncing..."
                    % playlist["title"]
                )
